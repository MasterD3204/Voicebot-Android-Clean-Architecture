package com.voicebot.data.rag.embedding
...
class EmbedRagEngine(
    ...
    companion object {
        ...
        // Collection name cho vector store
    }
    ...
    override suspend fun search(query: String): List<String>? {
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                // Tạo RetrievalRequest với query string và config
                val retrievalRequest = RetrievalRequest.create(
                    query,              // query (String)
                    retrievalConfig     // config
                )

                // Gọi retrieveResults (API thực tế)
                val response: RetrievalResponse<String> = semanticMemory!!
                    .retrieveResults(retrievalRequest)
                ...
                // RetrievalEntity chứa .getData()
                val topResults = entities
                    .mapNotNull { entity ->
                        val text = entity.getData()
